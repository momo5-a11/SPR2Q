...
import paddle.distributed.fleet.utils as checkpoint
...
def drop_path(x, drop_prob: float = 0.0, training: bool = False):
    ...
    output = x / paddle.to_tensor(keep_prob, dtype=x.dtype) * random_tensor
    return output
...
def check_bit(tensor, name):
    ...
    print(
        ...
        f"  - 是否为2-bit? (Is 2-bit?): {'✅ 是 (Yes)' if num_unique <= 4 else '❌ 否 (No)'}\n"
        f"-------------------------------------------"
    )
