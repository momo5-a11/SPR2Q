...
def charbonnier_loss(pred, target, eps=1e-12):
    return paddle.sqrt((pred - target)**2 + eps)
...
@LOSS_REGISTRY.register()
class CharbonnierLoss(paddle.nn.Layer):
        ...
        if self.perceptual_weight > 0:
            ...
            for k in x_features.keys():
                if self.criterion_type == 'fro':
                    percep_loss += paddle.linalg.norm(x_features[k] - gt_features[k], p='fro') * self.layer_weights[k]
                else:
...
def g_path_regularize(fake_img, latents, mean_path_length, decay=0.01):
    noise = paddle.randn(fake_img) / math.sqrt(fake_img.shape[2] * fake_img.shape[3])
    grad = paddle.grad(outputs=(fake_img * noise).sum(), inputs=latents, create_graph=True)[0]
...
def gradient_penalty_loss(discriminator, real_data, fake_data, weight=None):
    ...
    Args:
        discriminator (paddle.nn.Layer): Network for the discriminator.
    ...
    alpha = paddle.rand([batch_size, 1, 1, 1], dtype=real_data.dtype)
    ...
    if weight is not None:
        gradients_penalty /= paddle.mean(weight)

    return gradients_penalty


@LOSS_REGISTRY.register()
class GANFeatLoss(paddle.nn.Layer):
