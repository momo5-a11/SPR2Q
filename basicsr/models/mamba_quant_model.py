...
class MambaQuantModel(BaseModel):
    def __init__(self, opt):
        ...
        if self.opt['path']['pretrain_network_Q'] != None:
            ...
            self.be_ready_for_calibration()
            self.calibration()
        # print("====== [Step1] FakeQuantizer Parameters Check (Paddle) ======")
        # for name, module in self.net_Q.named_sublayers():
        #     if isinstance(module, FakeQuantizerBase):
        #         lb = getattr(module, 'lower_bound', None)
        #         ub = getattr(module, 'upper_bound', None)
        #         bit = getattr(module, 'n_bit', None)
        #         c = getattr(module, 'calibrated', None)
        #         lb_val = float(lb.mean().item()) if isinstance(lb, paddle.Tensor) else lb
        #         ub_val = float(ub.mean().item()) if isinstance(ub, paddle.Tensor) else ub
        #         bit_val = float(bit.mean().item()) if isinstance(bit, paddle.Tensor) else bit

        #         print(f"{name:<40} | lower={lb_val:.6f}, upper={ub_val:.6f}, n_bit={bit_val:.1f}, calibrated={c}")
        # print("==============================================================")
        # self.net_Q.eval()
        # with paddle.no_grad():
        #     dummy = paddle.ones([1, 3, 64, 64], dtype='float32')  
        #     out = self.net_Q(dummy)
        #     print(f"[Check] net_Q forward OK | shape={out.shape}, mean={out.mean().item():.6f}, std={out.std().item():.6f}")
        #     if paddle.isnan(out).any() or paddle.isinf(out).any():
        #         print("[Check] ⚠️ net_Q model output contains NaN or Inf!")
        #     else:
        #         print("[Check] ✅ net_Q model output numerically stable.")
        # print("[Exit] net_Q model test finished. Exiting program.")
        # sys.exit(0)
        # # ====================================================
    ...
    def build_quantized_lora_network(self):
        ...
        # self.quant_lora_conv2d: dict[str, QuantLoRA_QuantConv2d] = {}
        config = {
            'bit': self.opt['bit'],
        }
        ...
        for name, module in self.net_Q.named_sublayers():
            ...
            if isinstance(module, AttentiveLayer):
                ...
                module.wqkv = replace_linear(module.wqkv,  qkv=self.opt['quant']['qkv_separation'])                
                module.win_mhsa.proj = replace_linear(module.win_mhsa.proj)                
                module.convffn1.fc1 = replace_linear(module.convffn1.fc1)
                module.convffn1.fc2 = replace_linear(module.convffn1.fc2)                
                module.convffn2.fc1 = replace_linear(module.convffn2.fc1)
                module.convffn2.fc2 = replace_linear(module.convffn2.fc2)
                # assm
                module.assm.out_proj = replace_linear(module.assm.out_proj)
                module.assm.route[0] = replace_linear(module.assm.route[0])
                module.assm.route[2] = replace_linear(module.assm.route[2])

                # module.assm.selectiveScan.x_weight = FakeQuantizer_Lora_QuantWeight(module.assm.selectiveScan.x_weight,
                #                                                                     module.assm.selectiveScan.x_proj_weight,
                #                                                                     r=8)
                # module.assm.selectiveScan.dt_weight = FakeQuantizer_Lora_QuantWeight(module.assm.selectiveScan.dt_weight,
                #                                                                     module.assm.selectiveScan.dt_projs_weight,
                #                                                                     r=8)
                # module.assm.fq_embeddingB_w = FakeQuantizer_Lora_QuantWeight(module.assm.fq_embeddingB_w,
                #                                                                     module.assm.embeddingB.weight,
                #                                                                     r=8)
                # module.assm.fq_token_w = FakeQuantizer_Lora_QuantWeight(module.assm.fq_token_w,
                #                                                                     module.embeddingA.weight,
                #                                                                     r=8)
                
                if self.opt['quant']['qkv_separation']:
                    self.quant_lora_linears[f"{name}.wqkv.q"] = module.wqkv.q
                    self.quant_lora_linears[f"{name}.wqkv.k"] = module.wqkv.k
                    self.quant_lora_linears[f"{name}.wqkv.v"] = module.wqkv.v
                else:
                    self.quant_lora_linears[f"{name}.wqkv"] = module.wqkv
                self.quant_lora_linears[f"{name}.win_mhsa.proj"] = module.win_mhsa.proj
                
                self.quant_lora_linears[f"{name}.convffn1.fc1"] = module.convffn1.fc1
                self.quant_lora_linears[f"{name}.convffn1.fc2"] = module.convffn1.fc2
                
                self.quant_lora_linears[f"{name}.convffn2.fc1"] = module.convffn2.fc1
                self.quant_lora_linears[f"{name}.convffn2.fc2"] = module.convffn2.fc2
                # assm
                self.quant_lora_linears[f"{name}.assm.out_proj"] = module.assm.out_proj
                self.quant_lora_linears[f"{name}.assm.assm.route.0"] = module.assm.route[0]
                self.quant_lora_linears[f"{name}.assm.assm.route.2"] = module.assm.route[2]
                # assm weight-quantize
                # self.quant_lora_weight[f"{name}.assm.selectiveScan.x_weight"] = module.assm.selectiveScan.x_weight
                # self.quant_lora_weight[f"{name}.assm.selectiveScan.dt_weight"] = module.assm.selectiveScan.dt_weight
                # self.quant_lora_weight[f"{name}.assm.fq_embeddingB_w"] = module.assm.fq_embeddingB_w
                # self.quant_lora_weight[f"{name}.assm.fq_token_w"] = module.assm.fq_token_w

                # module.convffn1.sg.conv = replace_Conv2d(module.convffn1.sg.conv)
                # module.convffn2.sg.conv = replace_Conv2d(module.convffn2.sg.conv)
                # module.assm.in_proj[0] = replace_Conv2d(module.assm.in_proj[0])
                # module.assm.CPE[0] = replace_Conv2d(module.assm.CPE[0])
                # self.quant_lora_conv2d[f"{name}.convffn1.sg.conv"] = module.convffn1.sg.conv
                # self.quant_lora_conv2d[f"{name}.convffn2.sg.conv"] = module.convffn2.sg.conv
                # self.quant_lora_conv2d[f"{name}.assm.in_proj[0]"] = module.assm.in_proj[0]
                # self.quant_lora_conv2d[f"{name}.assm.CPE[0]"] = module.assm.CPE[0]
    ...
    def build_quantized_network(self):
        ...
        for name, module in self.net_Q.named_sublayers(include_self=True):

            if isinstance(module, AttentiveLayer):
                # 扫描模块量化
                module.assm.selectiveScan.dt_weight = FakeQuantizerWeight(self.opt['bit'])
                module.assm.selectiveScan.x_weight = FakeQuantizerWeight(self.opt['bit'])
                module.assm.selectiveScan.mymodule_xs = FakeQuantizerAct(self.opt['bit'])
                module.assm.selectiveScan.mymodule_dts = FakeQuantizerAct(self.opt['bit'])
                module.assm.selectiveScan.mymodule_delta = FakeQuantizerAct(self.opt['bit'])
                module.assm.selectiveScan.mymodule_Bs = FakeQuantizerAct(self.opt['bit'])
                module.assm.selectiveScan.mymodule_Cs = FakeQuantizerAct(self.opt['bit'])
                module.assm.selectiveScan.mymodule_As = FakeQuantizerAct(self.opt['bit'])
                module.assm.selectiveScan.mymodule_dt_projs_bias = FakeQuantizerAct(self.opt['bit'])
                module.assm.selectiveScan.mymodule_hs = FakeQuantizerAct(self.opt['bit'])

                self.quant_weight[f"{name}.assm.selectiveScan.dt_weight"] = module.assm.selectiveScan.dt_weight
                self.quant_weight[f"{name}.assm.selectiveScan.x_weight"] = module.assm.selectiveScan.x_weight
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_xs'] = module.assm.selectiveScan.mymodule_xs
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_dts'] = module.assm.selectiveScan.mymodule_dts
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_delta'] = module.assm.selectiveScan.mymodule_delta
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_Bs'] = module.assm.selectiveScan.mymodule_Bs
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_Cs'] = module.assm.selectiveScan.mymodule_Cs
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_As'] = module.assm.selectiveScan.mymodule_As
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_dt_projs_bias'] = module.assm.selectiveScan.mymodule_dt_projs_bias
                self.quant_act[f'{name}.assm.selectiveScan.mymodule_hs'] = module.assm.selectiveScan.mymodule_hs

                # 1. 替换 Attention 相关的线性层
                # wqkv 是三合一的 QKV 投影层，类似于 SwinIR
                module.wqkv = replace_linear(module.wqkv, config, qkv=self.opt['quant'].get('qkv_separation', True))
                module.win_mhsa.proj = replace_linear(module.win_mhsa.proj, config)

                # 2. 替换 GatedMLP (ConvFFN) 中的线性层
                module.convffn1.fc1 = replace_linear(module.convffn1.fc1, config)
                module.convffn1.fc2 = replace_linear(module.convffn1.fc2, config)
                module.convffn2.fc1 = replace_linear(module.convffn2.fc1, config)
                module.convffn2.fc2 = replace_linear(module.convffn2.fc2, config)

                # 3. 替换 ASSM 中的线性层
                module.assm.out_proj = replace_linear(module.assm.out_proj, config)

                # 替换self.route中的线性层
                module.assm.route[0] = replace_linear(module.assm.route[0], config)
                module.assm.route[2] = replace_linear(module.assm.route[2], config)

                # 4. 替换卷积层
                # GatedMLP 中有一个深度卷积
                module.convffn1.sg.conv = replace_conv(module.convffn1.sg.conv, config)
                module.convffn2.sg.conv = replace_conv(module.convffn2.sg.conv, config)
                # ASSM 中有两个卷积层
                # 注意: in_proj 是一个 Sequential，我们需要访问其内部的 Conv2d
                module.assm.in_proj[0] = replace_conv(module.assm.in_proj[0], config)
                module.assm.CPE[0] = replace_conv(module.assm.CPE[0], config)

                # 提示库生成
                module.assm.fq_embeddingB_w = FakeQuantizerWeight(self.opt['bit'])
                module.assm.fq_token_w = FakeQuantizerWeight(self.opt['bit'])
                module.assm.fq_cls_policy_a = FakeQuantizerAct(self.opt['bit'])
                module.assm.fq_full_embedding_a = FakeQuantizerAct(self.opt['bit'])

                self.quant_weight[f"{name}.assm.fq_embeddingB_w"] = module.assm.fq_embeddingB_w
                self.quant_weight[f"{name}.assm.fq_token_w"] = module.assm.fq_token_w
                self.quant_act[f"{name}.assm.fq_cls_policy_a"] = module.assm.fq_cls_policy_a
                self.quant_act[f"{name}.assm.fq_full_embedding_a"] = module.assm.fq_full_embedding_a

                # --- 记录所有被替换的量化模块 ---
                # 记录线性层
                if self.opt['quant'].get('qkv_separation', True):
                    self.quant_linears[f"{name}.wqkv.q"] = module.wqkv.q
                    self.quant_linears[f"{name}.wqkv.k"] = module.wqkv.k
                    self.quant_linears[f"{name}.wqkv.v"] = module.wqkv.v
                else:
                    self.quant_linears[f"{name}.wqkv"] = module.wqkv
                self.quant_linears[f"{name}.win_mhsa.proj"] = module.win_mhsa.proj
                self.quant_linears[f"{name}.convffn1.fc1"] = module.convffn1.fc1
                self.quant_linears[f"{name}.convffn1.fc2"] = module.convffn1.fc2
                self.quant_linears[f"{name}.convffn2.fc1"] = module.convffn2.fc1
                self.quant_linears[f"{name}.convffn2.fc2"] = module.convffn2.fc2
                self.quant_linears[f"{name}.assm.out_proj"] = module.assm.out_proj
                self.quant_linears[f"{name}.assm.route.0"] = module.assm.route[0]
                self.quant_linears[f"{name}.assm.route.2"] = module.assm.route[2]

                # 记录卷积层
                self.quant_conv[f"{name}.convffn1.sg.conv"] = module.convffn1.sg.conv
                self.quant_conv[f"{name}.convffn2.sg.conv"] = module.convffn2.sg.conv
                self.quant_conv[f"{name}.assm.in_proj"] = module.assm.in_proj[0]
                self.quant_conv[f"{name}.assm.CPE"] = module.assm.CPE[0]

                # 5. 注入用于qkv中间张量量化的伪量化器
                # 需要修改 mambairv2light_arch.py
            if isinstance(module, WindowAttention):
                ...
                module.mymodule_a = FakeQuantizerAct(self.opt['bit'])

                # 在 quant_act 字典中正确地登记它们
                ...
                self.quant_act[f'{name}.mymodule_a'] = module.mymodule_a

    def build_hooks_on_Q_and_F(self):
        # from basicsr.archs.mambairv2light_arch import BasicBlock

        self.feature_F = []
        self.feature_Q = []

        if self.opt["quant"]["hook_per_layer"]:
            hook_type = AttentiveLayer
