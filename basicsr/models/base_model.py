...
class BaseModel():
    ...
    def __init__(self, opt):
        self.opt = opt
        self.device = paddle.device('cuda' if opt['num_gpu'] != 0 else 'cpu')
    ...
    def model_to_device(self, net):
        """Model to device. It also wraps models with DataParallel (Paddle handles both single/multi GPU)."""
        # Paddle 会自动选择 GPU
        net = net.cuda() if self.device == 'gpu' else net

        if self.opt.get('dist', False):
            # 分布式训练初始化
            dist.init_parallel_env()
            net = paddle.DataParallel(net)
        elif self.opt.get('num_gpu', 1) > 1:
            # 多卡单机训练
            net = paddle.DataParallel(net)
        return net
    ...
    def get_optimizer(self, optim_type, params, lr, **kwargs):
        if optim_type == 'Adam':
            optimizer = paddle.optimizer.Adam(params, lr, **kwargs)
        else:
    ...
    def load_network(self, net, load_path, strict=True, param_key='params'):
        ...
        logger = get_root_logger()
        net = self.get_bare_model(net)
        load_net = paddle.load(load_path, map_location=lambda storage, loc: storage)
